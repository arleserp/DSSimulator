...
public class NetworkEnvironmentMASACO extends Environment implements Serializable {
    ...
    StatisticEngineInterface st;
     GenerateHashtableDataSet gds = new GenerateHashtableDataSet();

    private int agents_die = 0;
    FunctionComputingInterface F;
        ...
        if (flag) {
            ...
            switch (language.getActionIndex(act)) {
                case 0: // Initialize
                    ...
                    //Data 
                    int number = (Integer) a.getAttribute("input"); // (int) (Math.random() * 1000);
                    System.out.println("input: " + number + ", for agent " + a.getAttribute("ID"));
                    ...
                    a.setAttribute("ch_in", p.getAttribute("ch_in"));
                    a.setAttribute("ch_in_r", ch_in_r);
                    a.setAttribute("ch_out", p.getAttribute("ch_out"));
                    a.setAttribute("recv_ch", recv_ch);

                    a.setRound(a.getRound() + 1);
                    break;
                case 2: //Send shares new information to neighbors
                    ...
                    if ((((ArrayList) (a.getAttribute("ch_in"))).isEmpty()) && (((ArrayList) (a.getAttribute("ch_out"))).isEmpty())) {
                        ...
                        //controlBoard.getInstance().getStatisticsInteger(getExpectedRes());
                        a.die();
                        agents_die++;
                    } else {
                ...
                case 1: // Receive
                    ...
                    Hashtable recvi = HashtableOperations.deserializeHashtable(msgr);
                    chanintmp.remove(ch);
                    ...
                    if (chanout.contains(ch) && a.getAttribute("send") != null) {
                        ...
                        if (HashtableOperations.isContained(send, recvi)) {
                            //System.out.println("sendi is contained in recvi");
                            chanout.remove(ch);

                        }
                        if (HashtableOperations.isContained(recvi, send)) {
                            //System.out.println("sendi is contained in recvi");
                            chanin.remove(ch);
                        }
                    }
                    ...
                    ((Hashtable) a.getAttribute("recv_ch")).put(ch, recvi);

                    //Call function F
                    Object out = F.Compute((Hashtable) a.getAttribute("newi"), (Hashtable) a.getAttribute("infi"), recvi, (Hashtable) a.getAttribute("outi"), ch, (String) a.getAttribute("ID"));

                    //This is a new output to the control board. 
                    returnOutput((String) a.getAttribute("ID"), (Hashtable) a.getAttribute("outi"));

                    // newi = newi U (recvi \ infi)
                    Hashtable temp = HashtableOperations.DifferenceSets(recvi, (Hashtable) a.getAttribute("infi"));
            ...
            if (agents_die == this.agents.size()) {
                System.out.println("the end");
                if(st != null){
                    st.printStatistics(agents_die);
                }else{
                   
                    //pendiente ponerle nombre al archivo
                    gds.saveOutput();
                }
                System.exit(0);
            }
    ...
    public Percept sense(Agent agent) {
        ...
        ArrayList ch_in = new ArrayList(g.getOutEdges((String) anAgent.getAttribute("ID")));
        ArrayList ch_out = new ArrayList(g.getOutEdges((String) anAgent.getAttribute("ID")));

        ch_in.remove((String) anAgent.getAttribute("ID"));
        ch_out.remove((String) anAgent.getAttribute("ID"));
        p.setAttribute("ch_in", ch_in);
        p.setAttribute("ch_out", ch_out);
        p.setAttribute("neighbors", g.getNeighbors((String) anAgent.getAttribute("ID")));
        p.setAttribute("round", (Integer) anAgent.getRound());
        return p;
    }

    public NetworkEnvironmentMASACO(Vector<Agent> _agents, SimpleLanguage _language, Graph gr, FunctionComputingInterface f, String fn, StatisticEngineInterface stat) {
        super(_agents);
    ...
    public NetworkEnvironmentMASACO(Vector<Agent> _agents, SimpleLanguage _language, Graph gr) {
        super(_agents);
        //this.g = new SparseMultigraph();
        int n = _agents.size();
        for (int i = 0; i < n; i++) {
    ...
    public NetworkEnvironmentMASACO(Agent agent, SimpleLanguage _language, Graph gr) {
        super(agent);
        this.g = gr;
        language = _language;
    }

    public NetworkEnvironmentMASACO copy() {
        return new NetworkEnvironmentMASACO(agents, language, g);
    }
